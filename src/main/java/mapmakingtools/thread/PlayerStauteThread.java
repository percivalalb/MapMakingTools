                	...
                	for(int xMult = 0; xMult < this.multiplyier; xMult++) {
                		for(int yMult = 0; yMult < this.multiplyier; yMult++) {
                			for(int zMult = 0; zMult < this.multiplyier; zMult++) {
	    	                	List<Integer> ints = Arrays.asList(this.data.getMinX() + worldX * this.multiplyier + xMult, this.data.getMinY() + worldY * multiplyier + yMult, data.getMinZ() + worldZ * multiplyier + zMult);
	    	    				long t = System.currentTimeMillis();
	    	                	if(!alreadySet.contains(ints)) {
	    	    					CachedBlock undo = new CachedBlock(this.player.worldObj, data.getMinX() + worldX * this.multiplyier + xMult, data.getMinY() + worldY * multiplyier + yMult, data.getMinZ() + worldZ * multiplyier + zMult);
	    		     	            list.add(undo);
	    		     	            alreadySet.add(ints);
	    	    				}
	    	     	            
	    	                	this.player.worldObj.setBlock(this.data.getMinX() + worldX * this.multiplyier + xMult, this.data.getMinY() + worldY * multiplyier + yMult, data.getMinZ() + worldZ * multiplyier + zMult, Block.getBlockFromName((String)closest[0]), (Integer)closest[1], 2);
	    	                	long u = System.currentTimeMillis();
	    	                	LogHelper.info("Time: " +(u - t));
	    	                	++blocks;
                			}
	                	}
                	}
