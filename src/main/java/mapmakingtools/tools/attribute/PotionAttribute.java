...
public class PotionAttribute extends IItemAttribute {
	...
	public void onItemCreation(ItemStack stack, int data) {
		if(!Strings.isNullOrEmpty(this.level) && !Strings.isNullOrEmpty(this.duration) && this.selected != -1 && data == 0) {
			if(NumberParse.isInteger(this.level)) {
				...
				if(!stack.hasTagCompound() || !stack.stackTagCompound.hasKey("CustomPotionEffects", 9)) {
		...
		if(this.selectedDelete != -1 && data == 1) {
			if(stack.hasTagCompound() && stack.stackTagCompound.hasKey("CustomPotionEffects", 9)) {
		        NBTTagList nbttaglist = stack.stackTagCompound.getTagList("CustomPotionEffects", 10);
		...
		if(data == 2) {
			...
			if(!stack.stackTagCompound.hasKey("CustomPotionEffects", 9))
				stack.stackTagCompound.setTag("CustomPotionEffects", new NBTTagList());
				
			NBTTagList list = stack.stackTagCompound.getTagList("CustomPotionEffects", 10);
	...
	public void initGui(IGuiItemEditor itemEditor, int x, int y, int width, int height) {
		...
		this.fld_duration = new GuiTextField(itemEditor.getFontRenderer(), x + 59, y + height / 2 - 20, 50, 14);
