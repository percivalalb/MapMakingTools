...
public class ArmorColourAttribute extends IItemAttribute {
	...
	public void onItemCreation(ItemStack stack, int data) {
		...
		if(data == 1) {
			if(stack.hasTagCompound())
				if(stack.stackTagCompound.hasKey("display", 10))
					if(stack.stackTagCompound.getCompoundTag("display").hasKey("color", 3)) {
						stack.stackTagCompound.getCompoundTag("display").removeTag("color");
						if(stack.stackTagCompound.getCompoundTag("display").hasNoTags()) {
	...
	public void populateFromItem(IGuiItemEditor itemEditor, ItemStack stack, boolean first) {
		if(first)
			if(stack.hasTagCompound())
				if(stack.stackTagCompound.hasKey("display", 10))
					if(stack.stackTagCompound.getCompoundTag("display").hasKey("color", 3)) {
						int integer = stack.stackTagCompound.getCompoundTag("display").getInteger("color");
						this.fld_colourint.setText(String.valueOf(integer));
						this.fld_colourhex.setText(Integer.toHexString(integer));
					}
		if(!stack.hasTagCompound() || (!stack.stackTagCompound.hasKey("display", 10) || !stack.stackTagCompound.getCompoundTag("display").hasKey("color", 3))) {
	...
	public void initGui(IGuiItemEditor itemEditor, int x, int y, int width, int height) {
		this.fld_colourint = new GuiTextField(itemEditor.getFontRenderer(), x + 2, y + 28, 80, 13);
		this.fld_colourhex = new GuiTextField(itemEditor.getFontRenderer(), x + 86, y + 28, 80, 13) {
		    @Override
