...
public class ConvertToDispenserClientFilter extends IFilterClient {
	...
	public void actionPerformed(IGuiFilter gui, GuiButton button) {
		...
		if (button.enabled) {
            switch (button.id) {
                case 0:
                	MapMakingTools.NETWORK_MANAGER.sendPacketToServer(new PacketConvertToDispenser(gui.getX(), gui.getY(), gui.getZ()));
	...
	public List<String> getFilterInfo(IGuiFilter gui) {
		return TextHelper.splitInto(140, gui.getFont(), EnumChatFormatting.GREEN + this.getFilterName(), StatCollector.translateToLocal("mapmakingtools.filter.converttodispenser.info"));
	}
