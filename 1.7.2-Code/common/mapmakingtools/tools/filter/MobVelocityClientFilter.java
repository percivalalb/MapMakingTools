...
public class MobVelocityClientFilter extends IFilterClientSpawner {
	...
	public String getIconPath() {
		return "mapmakingtools:velocity";
	}
	...
	public void initGui(IGuiFilter gui) {
        ...
        this.txt_xMotion = new GuiTextField(gui.getFont(), topX + 20, topY + 37, 90, 20);
        this.txt_xMotion.setMaxStringLength(7);
        this.txt_xMotion.setText(xText);
        this.txt_yMotion = new GuiTextField(gui.getFont(), topX + 120, topY + 37, 90, 20);
        this.txt_yMotion.setMaxStringLength(7);
        this.txt_yMotion.setText(yText);
        this.txt_zMotion = new GuiTextField(gui.getFont(), topX + 60, topY + 72, 90, 20);
	...
	public void actionPerformed(IGuiFilter gui, GuiButton button) {
		...
		if (button.enabled) {
            switch (button.id) {
                case 0:
                	MapMakingTools.NETWORK_MANAGER.sendPacketToServer(new PacketMobVelocity(gui.getX(), gui.getY(), gui.getZ(), txt_xMotion.getText(), txt_yMotion.getText(), txt_zMotion.getText(), this.minecartIndex));
	...
	public List<String> getFilterInfo(IGuiFilter gui) {
		return TextHelper.splitInto(140, gui.getFont(), EnumChatFormatting.GREEN + this.getFilterName(), StatCollector.translateToLocal("mapmakingtools.filter.mobvelocity.info"));
	}
