	    			...
	    			for(int i = 0; i < BlockList.getListSize(); ++i) {
	                	...
	                	if(mouseX > 4 + 16 * column + renderOffset && mouseX < 4 + 16 * (column + 1) + renderOffset && mouseY > 4 + 16 * row && mouseY < 4 + 16 * (row + 1)) {
		    				...
		    				if(Mouse.isButtonDown(1) && hasButtonBeenUp) {
				    			String txtToInsert = String.format((ReflectionHelper.getField(GuiChat.class, GuiTextField.class, chat, 8).getText().endsWith(" ") ? "" : " ") +"%s %s ", Block.blockRegistry.getNameForObject(Block.getBlockFromItem(item.getItem())), item.getItemDamage());
				    			for(int s = 0; s < txtToInsert.length(); s++) {
				    				char ch = txtToInsert.charAt(s);
				    				ReflectionHelper.getField(GuiChat.class, GuiTextField.class, chat, 8).textboxKeyTyped(ch, Integer.valueOf(ch));
				    			}
				    			ReflectionHelper.getField(GuiChat.class, GuiTextField.class, chat, 8).setCursorPosition(chatPostion + txtToInsert.length());
