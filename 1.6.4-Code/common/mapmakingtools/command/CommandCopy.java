...
public class CommandCopy extends CommandBase {
    ...
    public String getCommandUsage(ICommandSender par1ICommandSender) {
        return "commands.build.copy.usage";
    }
		...
		for(int x = minX; x < maxX; ++x) {
			for(int y = minY; y < maxY; ++y) {
				for(int z = minZ; z < maxZ; ++z) {
					CachedBlockPlacement undo = new CachedBlockPlacement(var3.worldObj, x, y, z, var3);
					list.add(undo);
					++blocks;
				} 
			}
		}
		var3.addChatMessage(blocks + " block(s) copied.");
		DataStorage.addCopy(list, var3);
    }
