...
public class CommandRoof extends CommandBase {
    ...
    public String getCommandUsage(ICommandSender par1ICommandSender) {
        return "commands.build.roof.usage";
    }
    	...
    	if (par2ArrayOfStr.length == 1) {
			...
			for(int x = minX; x < maxX; ++x) {
				for(int z = minZ; z < maxZ; ++z) {
					CachedBlockPlacement undo = new CachedBlockPlacement(var3.worldObj, x, maxY - 1, z);
					list.add(undo);
					var3.worldObj.setBlock(x, maxY - 1, z, blockId, blockMeta, 2);
					++blocks;
				} 
			}
			...
			throw new WrongUsageException("commands.build.roof.usage", new Object[0]);
	    }
    }
