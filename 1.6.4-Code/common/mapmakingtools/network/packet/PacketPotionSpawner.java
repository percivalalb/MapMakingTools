...
public class PacketPotionSpawner extends PacketMMT {

	public int x, y, z;
	
	public PacketPotionSpawner() {
		super(PacketTypeHandler.POTION_SPAWNER, false);
	}
	
	public PacketPotionSpawner(int x, int y, int z) {
		this();
		this.x = x;
		this.y = y;
		this.z = z;
		...
		if(player instanceof EntityPlayerMP) {
			...
			if(playerMP.openContainer != null && playerMP.openContainer instanceof ContainerFilter) {
				...
				if(container.current != null && container.current instanceof FilterServerPotionSpawner) {
					...
					player.sendChatToPlayer(ChatMessageComponent.createFromTranslationKey("filter.potionSpawner.complete"));
				}
			}
		}
	}
