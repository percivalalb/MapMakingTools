...
public class SpawnerHelper {
	...
	public static MobSpawnerBaseLogic getSpawnerLogic(Object targetSpawner) {
		...
		if(targetSpawner instanceof TileEntityMobSpawner) return ((TileEntityMobSpawner)targetSpawner).getSpawnerLogic(); 
	...
	public static void sendSpawnerPacketToAllPlayers(Object targetSpawner) {
		if(targetSpawner instanceof TileEntityMobSpawner) {
			...
			server.getConfigurationManager().sendPacketToAllPlayersInDimension(spawner.getDescriptionPacket(), spawner.worldObj.provider.dimensionId);
			LogHelper.logDebug("Sent Packet to all players.");
		}
	}
	...
	public static void setMobId(Object tile, String mobId) {
		...
		if(tile instanceof TileEntityMobSpawner) {
			...
			spawner.worldObj.setBlockTileEntity(spawner.xCoord, spawner.yCoord, spawner.zCoord, newSpawner = new TileEntityMobSpawner());
			mobSpawnerLogic = getSpawnerLogic(newSpawner);
		}
		if(mobSpawnerLogic != null) { 
			LogHelper.logDebug("Set Spawner Mob Id to " + mobSpawnerLogic.getEntityNameToSpawn());
	...
	public static WeightedRandomMinecart getWeightedMinecart(Object tile) {
		...
		return new WeightedRandomMinecart(mobSpawnerLogic, new NBTTagCompound(), getMobId(tile));
	}
