 ...
 */
public class FilterConvertToDropper implements IFilter {
	...
	public String getFilterName() {
		return StatCollector.translateToLocal("filter.convertToDropper");
	}
	...
	public boolean isApplicable(EntityPlayer player, World world, int x, int y, int z) {
		...
		if(tile != null && tile instanceof TileEntityDispenser && tile.getClass() == TileEntityDispenser.class) {
			return true;
		}
		return false;
	}
	...
	public void initGui(GuiFilterMenu gui) {
        ...
        this.btn_covert = new GuiButton(0, k + 20, l + 37, 200, 20, "Convert to Dropper");
	...
	public void actionPerformed(GuiFilterMenu gui, GuiButton var1) {
		if (var1.enabled) {
            switch (var1.id) {
                case 0:
                	PacketTypeHandler.populatePacketAndSendToServer(new PacketConvertToDropper(gui.x, gui.y, gui.z));
                	ClientHelper.mc.displayGuiScreen(null);
                    ClientHelper.mc.setIngameFocus();
            }
        }
