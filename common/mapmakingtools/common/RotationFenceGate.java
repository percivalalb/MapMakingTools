...
public class RotationFenceGate implements IPasteRotate {
	...
	private static int[] EAST = new int[] {3, 7};
	private static int[] SOUTH = new int[] {0, 4};
	private static int[] WEST = new int[] {1, 5};
	...
	public void onPaste(CachedBlockPlacement cache, World world, int x, int y, int z, int rotation) {
		...
		world.setBlockMetadataWithNotify(x, y, z, directions[newDirection][isOpen ? 1 : 0], 2);
	}
