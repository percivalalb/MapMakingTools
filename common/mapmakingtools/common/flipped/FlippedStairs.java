...
public class FlippedStairs implements IPasteFlip {
	...
	private static int[] NORTH = new int[] {2, 6};
	private static int[] EAST = new int[] {1, 5};
	private static int[] SOUTH = new int[] {3, 7};
	private static int[] WEST = new int[] {0, 4};
	...
	public void onFlip(CachedBlockPlacement cache, World world, int x, int y, int z, int flipMode) {
		...
		if(flipMode == 0) {
			if(index2 == 1)
				world.setBlockMetadataWithNotify(x, y, z, directions[index][0], 2);
			else if(index2 == 0)
				world.setBlockMetadataWithNotify(x, y, z, directions[index][1], 2);
		}
