...
public class RotationPoweredRail implements IPasteRotate {

	private static int[] NORTH = new int[] {2, 10};
	private static int[] EAST = new int[] {5, 13};
	private static int[] SOUTH = new int[] {3, 11};
	private static int[] WEST = new int[] {4, 12};
	...
	public void onPaste(CachedBlockPlacement cache, World world, int x, int y, int z, int rotation) {
		...
		else if((meta == 8 || meta == 9) && (rotation == 90 || rotation == 270)) {
			world.setBlockMetadataWithNotify(x, y, z, meta == 8 ? 9 : 8, 2);
			return;
		}
		else if(meta == 0 || meta == 1 || meta == 8 || meta == 9) {
			world.setBlockMetadataWithNotify(x, y, z, meta, 2);
			return;
		}
        int direction = metaToListType(meta);
        boolean isPowered = meta >= 8 && meta <= 13;
		...
		world.setBlockMetadataWithNotify(x, y, z, directions[newDirection][isPowered ? 1 : 0], 2);
	}
