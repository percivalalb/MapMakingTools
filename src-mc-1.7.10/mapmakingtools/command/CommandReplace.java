...
public class CommandReplace extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] param) throws CommandException {
		...
		if(param.length < 4)
			...
			int targetMeta = parseIntBounded(sender, param[1], -1, 15);
			int replaceMeta = parseIntBounded(sender, param[3], 0, 15);
			...
			for(BlockPos pos : positions) {
				...
				if(rightBlock && rightMeta) { 
					list.add(bse);
					data.getPlayerWorld().setBlock(pos.getX(), pos.getY(), pos.getZ(), replaceBlock, replaceMeta, 2);
					blocks += 1;
				}
			}
