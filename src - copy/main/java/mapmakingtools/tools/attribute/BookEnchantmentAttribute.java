...
public class BookEnchantmentAttribute extends IItemAttribute {
	...
	public void onItemCreation(ItemStack stack, int data) {
		...
		if(this.selectedDelete != -1 && data == 1) {
			if(stack.hasTagCompound() && stack.stackTagCompound.hasKey("StoredEnchantments", 9)) {
		        NBTTagList nbttaglist = stack.stackTagCompound.getTagList("StoredEnchantments", 10);
		        nbttaglist.removeTag(this.selectedDelete);
		        if(nbttaglist.tagCount() == 0)
		        	stack.stackTagCompound.removeTag("StoredEnchantments");
			}
		}
		
		if(data == 2) {
			if(stack.hasTagCompound())
				if(stack.stackTagCompound.hasKey("StoredEnchantments", 9))
					stack.stackTagCompound.removeTag("StoredEnchantments");
		}
	}
	...
	public void populateFromItem(IGuiItemEditor itemEditor, ItemStack stack, boolean first) {
		...
		NBTTagList enchantmentList = stack.stackTagCompound.getTagList("StoredEnchantments", 10);
