...
public class BookAttribute extends IItemAttribute {
	...
	public void onItemCreation(ItemStack stack, int data) {
		if(this.name != null && data == 0) {
			...
			stack.stackTagCompound.setString("title", this.name);
		}
		...
		if(this.author != null && data == 1) {
			...
			stack.stackTagCompound.setString("author", this.author);
		}
	...
	public void populateFromItem(IGuiItemEditor itemEditor, ItemStack stack, boolean first) {
		if(first) {
			if(stack.hasTagCompound()) {
				if(stack.stackTagCompound.hasKey("title", 8)) {
					this.fld_name.setText(stack.stackTagCompound.getString("title"));
				}
				if(stack.stackTagCompound.hasKey("author", 8)) {
					this.fld_author.setText(stack.stackTagCompound.getString("author"));
				}
			}
		}
	}
	...
	public void initGui(IGuiItemEditor itemEditor, int x, int y, int width, int height) {
		this.fld_name = new GuiTextField(itemEditor.getFontRenderer(), x + 2, y + 28, 80, 13);
		this.fld_author = new GuiTextField(itemEditor.getFontRenderer(), x + 86, y + 28, 80, 13);
