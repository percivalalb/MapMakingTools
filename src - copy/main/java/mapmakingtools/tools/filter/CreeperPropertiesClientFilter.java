...
public class CreeperPropertiesClientFilter extends IFilterClientSpawner {
	...
	public String getIconPath() {
		return "mapmakingtools:explosion";
	}
	...
	public void initGui(IGuiFilter gui) {
        ...
        this.txt_radius = new GuiTextField(gui.getFont(), topX + 120, topY + 37, 90, 20);
        this.txt_radius.setMaxStringLength(7);
        this.txt_radius.setText(radiusText);
        this.txt_fuse = new GuiTextField(gui.getFont(), topX + 20, topY + 37, 90, 20);
		...
		if(tile instanceof TileEntityMobSpawner) {
			...
			//if(SpawnerUtil.isBabyMonster(spawner.func_145881_a(), this.minecartIndex)) {
	...
	public void actionPerformed(IGuiFilter gui, GuiButton button) {
		...
		if (button.enabled) {
            switch (button.id) {
                case 0:
                	MapMakingTools.NETWORK_MANAGER.sendPacketToServer(new PacketCreeperProperties(gui.getX(), gui.getY(), gui.getZ(), this.txt_fuse.getText(), this.txt_radius.getText(), this.minecartIndex));
